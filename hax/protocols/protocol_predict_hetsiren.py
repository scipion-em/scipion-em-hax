...
class JaxProtPredictHetSiren(ProtAnalysis3D, ProtFlexBase):
    """
    Predicts particle orientations, in-plane shifts, and latent heterogeneity coordinates using a previously trained
    HetSIREN neural network. The protocol does not train a new model; instead, it reuses a network generated by a
    prior HetSIREN angular alignment run and applies that learned representation to a new particle set. In practical
    cryo-EM workflows, this is mainly used when one wants to transfer an existing heterogeneity model to additional
    particles, characterize new datasets in the same latent space, or decode representative structural states from
    ...
    Predict HetSIREN (JaxProtPredictHetSiren) — User Manual
        ...
        This protocol applies an already trained HetSIREN model to a new set of particles in order to estimate their
        angular assignments, translational shifts, and latent-space coordinates. Unlike the original HetSIREN
        alignment protocol, this step does not perform optimization of network parameters. Instead, it loads a
        previously trained model and uses it in inference mode.

        From a biological perspective, this protocol is especially useful when a structural heterogeneity model has
        already been learned and needs to be projected onto additional particles. Typical use cases include applying a
        previously trained model to an independent dataset, characterizing newly selected particles in the same
        conformational landscape, or extending a heterogeneity analysis without retraining the full network.

        Inputs and General Workflow

        The protocol requires two principal inputs: a set of particles to predict and a previously executed HetSIREN
        training protocol. The latter provides not only the trained neural network weights but also the structural
        context used during training, including latent dimensionality, CTF treatment, reconstruction mode, box size,
        reference maps, and masking information.

        During execution, the protocol first prepares a metadata representation of the new particle set. It then
        reproduces the preprocessing assumptions of the original HetSIREN training. This includes optional particle
        downsampling, reference volume resizing, mask adaptation, and metadata generation.

        Maintaining consistency with the original training protocol is essential. Prediction is only biologically
        meaningful when the new particles are expressed in a representation compatible with the space in which the
        HetSIREN model was originally learned.

        Preprocessing and Geometric Consistency

        If the original HetSIREN protocol used particle downsampling, the prediction step automatically applies the
        same working box size. This ensures that the network receives particles with the same spatial dimensions that
        were used during training.

        When the original protocol included a reference volume, that volume is also resized to match the working box
        size. Likewise, if a reconstruction mask was provided during training, the same mask is converted and resized
        accordingly.

        If no mask was originally defined, the protocol generates a circular 3D mask automatically. Biologically,
        this provides a neutral spatial constraint that restricts the prediction space while avoiding unconstrained
        density interpretation.

        Prediction with a Trained HetSIREN Model

        Once preprocessing is complete, the protocol launches HetSIREN in prediction mode. The trained network is
        reloaded from the selected HetSIREN protocol and applied directly to the new particle set.

        The network predicts for every particle a latent-space coordinate, three Euler angles, and in-plane shifts.
        These outputs represent the position of each particle both in geometric alignment space and in the learned
        conformational manifold.

        If the original model was trained under mass-transport mode, implicit representation mode, or local
        reconstruction mode, those same assumptions are propagated automatically during prediction. This ensures that
        inference remains compatible with the original learned representation.

        CTF Treatment and Data Loading

        The protocol inherits the CTF handling strategy from the original HetSIREN training protocol. This is
        important because the neural network learned its internal representation under a specific image formation
        model. Using inconsistent CTF assumptions during prediction can reduce interpretability and may introduce
        systematic bias.

        Images may be loaded directly into RAM for maximum throughput or accessed through memory mapping from disk.
        When memory mapping is used, providing a fast SSD or NVMe scratch directory improves practical performance,
        especially for large datasets.

        Particle Output and Interpretation

        The main particle output is a new flexible particle set. Each output particle preserves the metadata of the
        original particle but gains three additional pieces of information.

        First, the protocol assigns a latent-space vector that places the particle within the learned heterogeneity
        landscape. Biologically, particles located near one another in latent space often correspond to similar
        conformational or structural states.

        Second, the protocol computes updated projection geometry from the predicted Euler angles and translational
        shifts. These values are converted into transformation matrices so that the output particle set can be used
        directly in downstream reconstruction or heterogeneity workflows.

        Third, the output particle set stores references to the trained HetSIREN model and, when available, the
        reference map, reference mask, and CTF treatment. This metadata allows the predicted particles to remain
        connected to the structural framework that generated them.

        Decoding Structural States from Latent Space

        A biologically important feature of this protocol is its ability to decode representative structural states
        from the predicted latent coordinates.

        After prediction, the latent vectors are clustered using k-means. The cluster centers define representative
        latent positions that summarize the main regions of the learned conformational manifold. These representative
        latent coordinates are then passed through the trained decoder network to generate a set of 3D volumes.

        Conceptually, these decoded maps correspond to representative structural states inferred from the new
        particle set. This can provide direct biological insight into dominant conformational states, structural
        transitions, or discrete heterogeneity patterns.

        Volume Output

        The decoded volumes are rescaled back to the original particle box size and assigned the proper sampling
        rate. The final output is a set of volumes representing the principal conformational states inferred from the
        predicted particles.

        In biological interpretation, these maps should be viewed as representative latent-space states rather than
        strict class averages. They summarize the geometry of the learned manifold and can therefore reveal
        structural trends, continuous transitions, or dominant heterogeneity modes.

        Validation and Practical Recommendations

        The protocol validates that the reference mask inherited from the original HetSIREN training is binary.
        Binary masks are essential because the network expects a well-defined spatial support. Non-binary masks may
        lead to ambiguous geometric constraints and unreliable predictions.

        In practical cryo-EM analysis, the most meaningful results are obtained when the new particle set is
        biologically comparable to the data used for training. Applying a HetSIREN model to strongly unrelated
        particles may still generate latent coordinates, but those coordinates may not correspond to meaningful
        structural variability.

        Final Perspective

        This protocol acts as an inference bridge between a trained HetSIREN heterogeneity model and new particle
        data. Rather than learning a new conformational landscape, it projects additional particles into an existing
        one. For biological users, this enables direct comparison between datasets, extension of heterogeneity
        analyses, and reconstruction of representative conformational states without repeating the full training
        process.
    """
